...
def main():
    ...
    enc = ImpalaEncoder(
        width=1,
        stack_sizes=(16, m.henc, m.dstc),
        num_blocks=2,
        dropout_rate=None,
        layer_norm=False,
        input_channels=m.dobs,
        final_ln=True,
        mlp_output_dim=512,
        input_shape=(m.dobs, data_config.img_size, data_config.img_size),
    )
    f = enc(torch.rand(1, m.dobs, 1, data_config.img_size, data_config.img_size)).shape[
        1
    ]
    ...
    reg = VC_IDM_Sim_Regularizer(
        cov_coeff=m.regularizer.cov_coeff,
        std_coeff=m.regularizer.std_coeff,
        sim_coeff_t=m.regularizer.sim_coeff_t,
        idm_coeff=m.regularizer.get("idm_coeff", 0.1),
        idm=idm,
        first_t_only=m.regularizer.get("first_t_only"),
        projector=None,
        spatial_as_samples=m.regularizer.spatial_as_samples,
        idm_after_proj=m.regularizer.idm_after_proj,
        sim_t_after_proj=m.regularizer.sim_t_after_proj,
    )
    ...
    print(
        f"[cotrain] f={f} N={N} epochs={epochs} | joint fine-tune (shared latent)",
        flush=True,
    )
    ...
    opt = AdamW(
        [
            ...
            {
                "params": list(xy_head.parameters()) + list(subgoal.parameters()),
                "lr": 1e-3,
            },
        ],
        weight_decay=1e-5,
    ...
    print(
        f"[cotrain] staged: {freeze_epochs} heads-only epochs, then encoder lr={enc_lr}",
        flush=True,
    )
    ...
    for epoch in range(epochs):
        ...
        for x, a, loc, _, _ in loader:
            ...
            nb += 1
        print(
            f"[cotrain] epoch {epoch} {time.time()-t0:.0f}s jepa={jl/nb:.4f} "
            f"subgoal={sgl/nb:.5f} aux={al/nb:.5f}",
            flush=True,
        )
        save_checkpoint(
            Path(out_dir) / "latest.pth.tar",
            model=jepa,
            epoch=epoch,
            xy_head_state_dict=xy_head.state_dict(),
        )
        torch.save(
            {"subgoal": subgoal.state_dict(), "N": N, "f": f},
