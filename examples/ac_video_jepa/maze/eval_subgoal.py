...
def main():
    ...
    )  # render GIFs for first n_gifs eps
    ...
    def obs_tensor(o):
        return (
            norm.normalize_state(o.to(dtype=torch.float32, device=device))
            .unsqueeze(0)
            .unsqueeze(2)
        )
    ...
    print(
        f"[subgoal-eval] lookahead={lookahead} revisit_pen={revisit_pen} "
        f"budget={budget_factor}xA*+{budget_margin}",
        flush=True,
    )
    successes = []
    spls = []
    ...
    for ep in range(num_ep):
        ...
        for step in range(max_steps):
            ...
            for dd in range(4):
                zf = fine_kstep_target(
                    jepa, ot, torch.tensor([dd], device=device), lookahead, cell_size
                )
            ...
            cand = [
                d for d in order if d not in blocked.get(cell, set()) and d != last_rev
            ]
            cand += [d for d in order if d not in cand]
            if verbose and step < 12:
                print(
                    f"   [s{step}] cell={list(cell)} goal={env.goal_cell.tolist()} "
                    f"dist[D,U,R,L]={[round(x,2) for x in dist]}",
                    flush=True,
                )
            moved = False
            done = False
        ...
        if verbose:
            print(
                f"   ep0: A*_len={astar_len} budget={max_steps} moves={n_moves}",
                flush=True,
            )
        if ep < n_gifs and len(frames) > 1:
            ...
            try:
                save_gif(
                    torch.stack([f.to(torch.float32) for f in frames]),
                    os.path.join(rdir, f"ep{ep}_{label}.gif"),
                    fps=8,
    ...
    json.dump(
        {
            ...
            "N": sck["N"],
            "astar_free": True,
            "budget": f"{budget_factor}xA*+{budget_margin}",
        },
        open(os.path.join(rdir, "subgoal_eval.json"), "w"),
        indent=2,
    )
    print(
        f"[subgoal-eval] A*-FREE success={sr*100:.2f}%  SPL={spl:.3f}  over {num_ep} mazes "
        f"(budget {budget_factor}xA*+{budget_margin})",
        flush=True,
    )
