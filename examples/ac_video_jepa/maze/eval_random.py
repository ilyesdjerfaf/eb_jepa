...
def main():
    ...
    print(
        f"[random-eval] random-persist baseline | budget {factor}xA*+{margin}",
        flush=True,
    )
    ...
    for ep in range(num_ep):
        ...
        print(
            f"[random-eval] ep {ep}: {'SUCCESS' if success else 'fail'} "
            f"(A*={astar_len} budget={max_steps} moves={n_moves})",
            flush=True,
        )
    ...
    json.dump(
        {
            "policy": "random-persist",
            "success_rate": sr,
            "spl": spl,
            "num_episodes": num_ep,
            "budget": f"{factor}xA*+{margin}",
            "astar_free": True,
        },
        open(os.path.join(rdir, "random_eval.json"), "w"),
        indent=2,
    )
    print(
        f"[random-eval] RANDOM baseline: success={sr*100:.2f}%  SPL={spl:.3f}  over {num_ep} mazes",
        flush=True,
    )
