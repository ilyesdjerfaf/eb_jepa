...
def build_jepa(cfg, device):
    ...
    return JEPA(encoder, encoder, predictor, regularizer, SquareLossSeq(projector)).to(
        device
    )
...
def run(
    ...
    **overrides,
    ...
    log_config(cfg)
    logger.info(
        f"Training {cfg.optim.epochs} epochs on {len(train_set)} clips -> {exp_dir}"
    )
    ...
    for epoch in range(cfg.optim.epochs):
        ...
        last = {}
        for batch in tqdm(
            loader, desc=f"epoch {epoch}", disable=cfg.logging.get("tqdm_silent", True)
        ):
        ...
        metrics = {
            ...
            "train/vc_loss": last["vc"],
            "train/pred_loss": last["pred"],
            "train/std_loss": last["std_loss"],
        ...
        log_epoch(
            epoch,
            {"loss": last["loss"], "vc": last["vc"], "pred": last["pred"]},
    ...
    if wandb_run:
        ...
        wandb.finish()
    logger.info(
        f"done -> {exp_dir}/latest.pth.tar  (now run eval.py to probe the energy gap)"
    )
