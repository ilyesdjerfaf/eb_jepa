...
def main():
    ...
    base_s = (
        float(np.std(base_vals) / max(1, np.sqrt(len(base_vals)))) if base_vals else 0.0
    )

    os.makedirs(args.out_dir, exist_ok=True)
    factors = [f for f in FACTORS if f in data]
    fig, axes = plt.subplots(
        1, len(factors), figsize=(5 * len(factors), 4.2), squeeze=False
    )
    out = {
        "n_seeds": len(args.sweeps),
        "baseline_pct": round(base_m, 2),
        "baseline_sem_pct": round(base_s, 2),
        "factors": {},
    }
    for ax, factor in zip(axes[0], factors):
        ...
        ax.set(
            xlabel=f"{xlabel}\nbaseline={base_v}; {note}",
            ylabel="planning success (%)",
            title=factor,
            ylim=(-3, 103),
        )
        ax.grid(alpha=0.3)
    ...
    fig.suptitle(
        f"Two Rooms planning success vs factor-of-variation severity "
        f"({len(args.sweeps)} seeds, mean±SEM)",
        y=1.02,
    )
    fig.tight_layout()
    fig.savefig(
        os.path.join(args.out_dir, "factors_compare.png"), dpi=110, bbox_inches="tight"
    )
