...
def run(
    ...
    loader, val_loader, data_config, _ = init_data(
        ...
        cfg_data={**base_data, "pipeline": {"mode": "online"}},
        device=device,
    )
    ...
    for point in ev["grid"]:
        ...
        def env_creator(_cfg=env_config):
            return create_env(
                cfg.data.env_name,
                config=_cfg,
                n_allowed_steps=ev["n_allowed_steps"],
                level=ev["level"],
            )
        ...
        res = launch_plan_eval(
            jepa,
            env_creator,
            model_folder / "fov_eval",
            0,
            0,
            suffix=f"_{label}",
            num_eval_episodes=ev["num_episodes"],
        ...
        results.append(row)
        logger.info(
            f"[{label}] success={row['success_rate']:.3f} dist={row['mean_state_dist']:.2f}"
        )
